...
async def download_url(url, value, now_uuid):
    if not is_same_action_uuid(now_uuid, 'checkAlive'):
        return
    content = put_async_request_queue(url, 'ping')
    if content:
...
def fetch_url3(url, passwordDict, filenameDict):
    try:
        ...
        try:
            ...
            pass
        # file_details = put_async_request_queue(url, 'content', header=bili_header)
        # response = download_with_proxy(url, bili_header, 1, True, True, False)
        # response.raise_for_status()  # 如果响应的状态码不是 200，则引发异常
        # 源文件是二进制的AES加密文件，那么通过response.text转换成字符串后，数据可能会被破坏，从而无法还原回原始数据
        m3u_string = put_async_request_queue(url, 'content', header=bili_header)
...
def grab_normal_qiumihui(m3u_dict, mintimeout, maxTimeout, source_type, dict_single):
    ...
    data = put_async_request_queue(m3u8_url, 'json', header=bili_header, timeout=mintimeout)
    if not data:
        data = put_async_request_queue(m3u8_url, 'json', header=bili_header, timeout=maxTimeout)
    if not data:
        return None
...
def get_resolution2(liveurl, mintimeout, maxTimeout):
    playlist_text = put_async_request_queue(liveurl, 'text', header=bili_header, timeout=mintimeout)
    if not playlist_text:
        playlist_text = put_async_request_queue(liveurl, 'text', header=bili_header, timeout=maxTimeout)
    if not playlist_text:
        return None
...
def get_best_youtube_live(id, mintimeout, maxTimeout):
    ...
    url = youtubeUrl + id
    content = put_async_request_queue(url, 'text', header=bili_header, timeout=mintimeout)
    if not content or '.m3u8' not in content:
        content = put_async_request_queue(url, 'text', header=bili_header, timeout=maxTimeout)
    ...
    while True:
        if 'https://' in content[end - tuner: end]:
            ...
            match = get_resolution2(link[start: end], mintimeout, maxTimeout)
            if match:
...
def get_m3u8_link(id):
    ...
    response = put_async_request_queue(url, 'text', header=headers)
    if response:
        try:
            content = response  # 解码为字符串
        except Exception as e:
            print(e)
        # 使用正则表达式提取file对应的字符串
        pattern = r'file:\s*"(.*?)"'
...
def get_ts_data(id, number):
    ...
    response = put_async_request_queue(url, 'threaddownload', header=headers, threadnum=5)
    if response:
        return response
    else:
        return None
...
def get_m3u8_raw_content(url, id):
    ...
    response = put_async_request_queue(url, 'read', header=headers)
    if response:
        content = response
        new_m3u8_data = []
        ip = init_IP()
        ...
        for line in content.splitlines():
            if line.startswith(
                    (b'#EXTM3U', b'#EXT-X-VERSION:', b'#EXT-X-MEDIA-SEQUENCE', b'#EXT-X-TARGETDURATION', b'#EXTINF')):
                new_m3u8_data.append(line)
            else:
                try:
                    old = int(line.split(b'.')[0]) / 1000
                except:
                    old = 0
                    return None
                # num = now - old
                ...
                # else:
                #     new_m3u8_data.pop()
        if len(new_m3u8_data) == 0 or new_m3u8_data[-1].startswith(
                b'#EXT-X-TARGETDURATION') or b'#EXTM3U' not in new_m3u8_data:
            return None
        returndata = b'\n'.join(new_m3u8_data)
...
def get_m3u8_data_by_hkid(hkid):
    ...
    if old_m3u8_data_hk['end'] == '1':
        return None
    return old_m3u8_data_hk['newdata']
...
def serve_files_normal(filename):
    ...
    if id == 'qiumihui,':
        ...
        with hkdtmb_lock:
            hkid = id.split(',')[1]
            if old_m3u8_data_hk['id'] != hkid:
                stupid_has_pushed_ts.clear()
            # 强制换id，打断ts推送
            old_m3u8_data_hk['id'] = hkid
